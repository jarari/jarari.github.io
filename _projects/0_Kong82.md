...
![FX Editor](assets/kong82_7.png "FX Editor")

### 작업 배경

- 기존 유닛 FX 시스템은 풀링 구조와 연출 워크플로우 측면에서 개선 여지가 있었음

### 요구 스펙

- FX 아티스트가 게임 실행 없이도 애니메이션과 FX 싱크를 타임라인 상에서 검수·조정할 수 있는 툴
- 공격 애니메이션에 연동되는 투사체 생성, 비행, 충돌 및 폭발 FX의 전 과정을 에디터 내에서 확인하고 제어 가능
- skeleton 기반 FX 부착 노드 추가를 통해, 특정 시점에 원하는 위치에서 FX를 재생하거나 bone 추적이 가능하도록 하는 구조

### 구현 방식

- 툴 UI는 코딩 AI 에이전트를 활용하여 빠르게 개발하고, FX 아티스트의 피드백을 수용하며 개선함
    - 전반적인 UX는 타임라인 에디터를 참고하여 구성
    - 아티스트가 소켓과 FX 이벤트를 직접 정의하고, 애니메이션과의 싱크를 실시간으로 확인하며 작업할 수 있도록 함
    - 입력 가능한 항목들을 직렬화하여 작업 공간을 json 파일로 저장/로드할 수 있게 함
- FX 재생은 기존 프로토타입의 런타임 FX 재생 흐름을 유지하는 방향으로 설계
    - 애니메이션 재생 상태와 동기화하여 다음 FX 이벤트 정보를 불러오고, 정확한 타이밍에 출력되도록 처리
- 데이터를 저장하는 방식에서 I/O 액세스와 메모리 사용량은 최소화하면서 정확한 타이밍과 위치에 FX 이벤트를 재생할 수 있도록 구조적인 고민이 요구되었음
    - 소켓 명칭과 계층 구조 표현을 위해 문자열 데이터 저장이 필요
    - 다만 바이너리 파싱 시 가변 길이 문자열은 직접 다루기 불편하므로, 문자열을 각 위치에 바로 저장하지 않고 파일 마지막에 스트링 테이블을 두고 인덱스로 참조하는 구조를 사용
    - 각 유닛에 대한 바이너리 데이터를 각각 저장하는 방식 대신, 하나의 압축된 아카이브에 몰아서 저장하는 방식을 적용
    - 이를 통해 스트링 테이블 압축 및 중복 문자열 제거가 가능해졌고, I/O 액세스는 1회로 줄임
    - 최종적으로 유닛 타입 1기 기준 바이너리 데이터 크기를 약 200바이트 수준으로 관리
- 헤더에는 파싱용 매직값 K8FX, 버전, 인덱스 테이블 항목 수, 파일 내 스트링 테이블의 오프셋, 스트링 테이블 길이, 인덱스 테이블 (엔트리 해시, 오프셋, 길이) 정보를 저장
- 엔트리는 소켓→이벤트의 순서로 저장
    - 이벤트 데이터 크기를 줄이기 위해 이동/회전/크기 변환 존재 여부를 1바이트 플래그로 관리
    - 이동값은 half3
    - 회전값은 0~360 범위를 ushort(0~65535)로 매핑한 PackedRotation3
    - 크기값은 vector3
    - 지속 시간은 half를 사용

### 효과

- 프리팹에 FX를 그대로 부착하던 기존 구조 대신, 자연스러운 FX 풀링 구조가 적용되어 성능에 따른 파티클 수 관리가 수월해짐
- FX 아티스트가 의도한 대로 FX가 인게임에서 1:1로 출력됨
